...
func (j *jsiiProxy_DatabaseInstanceChildInstanceRefsList) validateSetInternalValueParameters(val interface{}) error {
	...
	case cdktn.IResolvable:
		// ok
	case *[]*DatabaseInstanceChildInstanceRefs:
		val := val.(*[]*DatabaseInstanceChildInstanceRefs)
	...
	case []*DatabaseInstanceChildInstanceRefs:
		val_ := val.([]*DatabaseInstanceChildInstanceRefs)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *[]*DatabaseInstanceChildInstanceRefs; received %#v (a %T)", val, val)
		}
	}
