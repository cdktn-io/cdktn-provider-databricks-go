...
func (j *jsiiProxy_AccessControlRuleSetGrantRulesList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*AccessControlRuleSetGrantRules:
		val := val.(*[]*AccessControlRuleSetGrantRules)
	...
	case []*AccessControlRuleSetGrantRules:
		val_ := val.([]*AccessControlRuleSetGrantRules)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *[]*AccessControlRuleSetGrantRules; received %#v (a %T)", val, val)
		}
	}
