...
func (i *jsiiProxy_InstancePool) validatePutPreloadedDockerImageParameters(value interface{}) error {
	...
	case cdktn.IResolvable:
		// ok
	case *[]*InstancePoolPreloadedDockerImage:
		value := value.(*[]*InstancePoolPreloadedDockerImage)
	...
	case []*InstancePoolPreloadedDockerImage:
		value_ := value.([]*InstancePoolPreloadedDockerImage)
		value := &value_
		for idx_cd4240, v := range *value {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter value[%#v]", idx_cd4240) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(value) {
			return fmt.Errorf("parameter value must be one of the allowed types: cdktn.IResolvable, *[]*InstancePoolPreloadedDockerImage; received %#v (a %T)", value, value)
		}
	}
